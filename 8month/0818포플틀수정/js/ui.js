...
$(document).ready(function () {
  ...
  }); // 선택한 li태그가 .book_roll 클래스 중 마지막 li 태그 뒤로 이동한다.
  // 롤링배너 오른쪽버튼
  $(".roll_right").click(function () {
    //가장 마지막 태그가 이동되도록 eq(-1)
    $(".book_roll li").eq(-1).insertBefore(".book_roll li:first-child");
  }); //선택한 li태그가 .book_roll 클래스 중 가장 마지막 요소가 첫 번째 요소
}); //.book_roll li:first-child(첫번째요소)가 앞에 삽입되도록 이동위치를 정한다.
// 제작순서 1. 왼쪽버튼을 클릭한다 click()
// 2. 첫번째 도서가 있는 <li>태그가 마지막 8번째 <li>태그 뒤로 요소를 이동한다.
// 첫 번째 <li>태그 찾기 .eq(index)
// 8번째 <li>태그 뒤로 요소 이동하기 A.insertAfter(B)
// 조건에 일치하는 요소뒤에 해당요소를 삽입합니다. B 뒤에 A를 추가 하라는 의미이다.
