...
// 그래프 그리기 함수
function drawGraph1() {
  // getCountext("2d") : 랜더링 컨텍스트와 그리기 함수를 사용할 수 있게하는 메소드
  // 캔버스의 랜더링 컨텐스트를 가져온다.
  ...
  ctx.font = "14px Arial"; //폰트크기, 폰트 종류
  ...
  // 눈금선
  ctx.font = "12px Arial"; //폰트크기, 폰트 종류
  ctx.textAlign = "right"; //글자 오른쪽 정렬
  ctx.textBaseline = "middle"; //글자 중앙을 가로지르는 가로줄
  for (var i = 0; i <= data.max; i += 5) {
    //데이터 최대값 만큼 반복 (5씩 증가)
    var y = 65 + i * 4.5; //가로줄 간격
    ctx.moveTo(50, y); //선 그리기 시작점 (x좌표, y좌표)
    ctx.lineTo(graph1.width - 20, y); // 선 그리기 끝 점 (x좌표, y좌표)
  ...
  for (var i = 0; i < data.data.length; i++) {
    ...
    ctx.fillRect(x, y + 20, 10, 10); // 10 * 10 사각형 그림
    ctx.font = "11px Arial";
    ctx.textAlign = "left";
    ctx.fillStyle = "black";
